...
public class MemberService {
	...
	public void saveOrUpdate(MemberRequest req) {
		...
		Member member = memberRepo.findByMobile(req.getMobile()).orElse(new Member());
		...
		// Calculate expiry
		member.setExpiryDate(calculateExpiry(
				member.getExpiryDate() != null ? member.getExpiryDate() : req.getStartDate(), req.getPlan()));
	...
	public List<MemberResponse> getAll(String filter) {
		...
		return members.stream()
				.map(m -> new MemberResponse(m.getId(), m.getName(), m.getMobile(), m.getPlan(),
						m.getActive() ? "Active" : "Expired", m.getPin(), m.getAge(), m.getStartDate(),
						m.getExpiryDate()))
				.collect(Collectors.toList());
	}

	public Member getByMobile(String mobile) {
		return memberRepo.findByMobile(mobile).orElseThrow(() -> new RuntimeException("Member not found"));
	}
